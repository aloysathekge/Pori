...
| GW-4 | Add SSE streaming to the API over the existing `PoriEvent` contract (`asyncio.Queue` + `call_soon_threadsafe`, keepalive timeout, client-disconnect → interrupt) | `pori/api/` (new stream endpoint), `pori/observability/events.py` | B4 (Tier i) | M / High | UNBLOCKED (2026-07-03) — pori/api repaired + smoke-tested; SSE now buildable |

...
class TestFilesystemTools:
    ...
    def test_write_file_success(self, temp_dir):
        ...
        params = WriteFileParams(file_path=str(test_file), content="Hello from test!")
        context = {}
    ...
    def test_list_directory_recursive(self, sample_files):
        ...
        params = ListDirectoryParams(
            directory_path=str(sample_files), recursive=True, max_depth=2
        )
        context = {}
    ...
    def test_search_files_pattern(self, sample_files):
        ...
        params = SearchFilesParams(directory_path=str(sample_files), pattern="*.py")
        context = {}
    ...
    def test_search_files_content(self, sample_files):
        ...
        params = SearchFilesParams(
            directory_path=str(sample_files), pattern="*", content_search="Hello"
        )
        context = {}
...
def test_filesystem_tools_integration():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        original_is_safe = fs_config.is_path_safe
        fs_config.is_path_safe = (
            lambda x, **kwargs: tmp_path in Path(x).parents or Path(x) == tmp_path
        )
        ...
        try:
            ...
            write_result = write_file_tool(
                WriteFileParams(
                    file_path=str(test_file), content="Integration test content"
                ),
                context,
            )
            ...
            search_result = search_files_tool(
                SearchFilesParams(
                    directory_path=str(tmp_path), pattern="integration_test.txt"
                ),
                context,
            )

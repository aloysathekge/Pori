...
slice is committed at `8a868e9`; it adds bounded, durable Conversation context
compaction and scoped Event-history page faults. The fourth slice is implemented
and uncommitted: tools can declare a separate read-only provider reconciler;
uncertain Proposal outcomes are leased, backed off, and resolved only from
provider evidence. Gmail and Calendar now carry deterministic attempt
correlation, and the provider-success/database-crash drill commits the recovered
receipt without a second send. Verification currently passes `45` focused
Proposal/Google/migration tests, `55` adjacent approval/Event/Surface/worker
regressions, the complete kernel suite (`627 passed, 1 skipped`), and kernel
plus backend mypy (`110` and `124` source files). Live University, Madrid, and
Career model acceptance remains deferred while provider credits are
unavailable.
...
- Provider reconciliation is an explicit read-only tool capability, never an
  alternate execution path. Unknown lookup results retain uncertainty and back
  off; only typed provider evidence can commit or fail the Proposal.
- Gmail new-send recovery uses a deterministic RFC822 Message-ID and Calendar
  creation uses a deterministic provider event ID. `gmail_send_draft` remains
  at-most-once with explicit uncertainty until its adapter has an equally strong
  lookup key.
...
Review and commit R8's provider reconciliation slice, then complete responsive /
accessibility QA at the required Event workspace and generated-Surface
viewports. Keep

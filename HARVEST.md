...
|---|---|---|---|---|---|
| 2026-07-16 | Storage-only virtual filesystem contract with typed results, longest-prefix composite mounts, read-only routes, paginated reads, and an adapter over Pori's existing sandbox files | Deep Agents `libs/deepagents/deepagents/backends/{protocol.py,composite.py,sandbox.py}` and `libs/deepagents/deepagents/middleware/filesystem.py` @ `9fdd498937842e2ac490ae0e0ca74eef47abd0bb` | MIT | `pori/file_backends.py`, `pori/tools/standard/filesystem_tools.py`, `pori/orchestrator/core.py` | Give an Aloy run explicit `/event` and `/workspace` capabilities without LangGraph or a Deep Agents runtime dependency. Pori deliberately denies unmatched mounts and does **not** adopt the donor's host-filesystem or local-shell backends; command execution remains a separate `Sandbox` capability. |

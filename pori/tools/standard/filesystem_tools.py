...
class FilesystemConfig:
    ...
    def is_path_safe(
        ...
        try:
            ...
            current_relative = (
                self.current_dir in path.parents or path == self.current_dir
            )
...
class SearchFilesParams(BaseModel):
    ...
    content_search: Optional[str] = Field(
        None, description="Search for content within files"
    )
...
def _resolve_sandbox_path_if_needed(
    ...
    if (
        not thread_id
        or not base_dir
        or get_thread_data is None
        or replace_virtual_path is None
    ):
        return path, None
    ...
    def wrapper(params, context: Dict[str, Any]):
        try:
            ...
            if hasattr(params, "file_path"):
                ...
                src, sandbox_base = _resolve_sandbox_path_if_needed(
                    params.source_path, context
                )
                if sandbox_base is not None:
                    params.source_path = src
                    extra_bases = [sandbox_base]
                if not fs_config.is_path_safe(
                    params.source_path, extra_allowed_bases=extra_bases
                ):
                    return {
                        "success": False,
                        "error": f"Source path not allowed: {params.source_path}",
                    }
                dst_path, dst_base = _resolve_sandbox_path_if_needed(
                    params.destination_path, context
                )
                ...
                if not fs_config.is_path_safe(
                    params.destination_path, extra_allowed_bases=extra_bases
                ):
                    return {
                        "success": False,
                        "error": f"Destination path not allowed: {params.destination_path}",
                    }
...
def read_file_tool(params: ReadFileParams, context: Dict[str, Any]):
    ...
    try:
        ...
        if path.stat().st_size > fs_config.max_file_size:
            return {
                "success": False,
                "error": f"File too large (max {format_bytes(fs_config.max_file_size)})",
            }
        ...
        return {
            "success": False,
            "error": f"Cannot decode file with encoding {params.encoding}",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def list_directory_tool(params: ListDirectoryParams, context: Dict[str, Any]):
    ...
    try:
            ...
            try:
                for item in sorted(dir_path.iterdir()):
                    ...
                    if (
                        params.recursive
                        and item.is_dir()
                        and current_depth < params.max_depth
                    ):
        ...
        logger.info(
            f"Listed directory: {path} ({len(files)} files, {len(directories)} directories)"
        )
    ...
    try:
        ...
        for match in path.rglob(pattern):
            ...
            if (
                ...
                and match.is_file()
                and fs_config.is_extension_allowed(match)
            ):
                try:
                    with open(match, "r", encoding="utf-8", errors="ignore") as f:
                        ...
                        if params.content_search.lower() in content.lower():
                            ...
                            result_item["content_matches"] = matching_lines[
                                :5
                            ]  # First 5 matches
        ...
        logger.info(
            f"Search completed: {len(results)} results for pattern '{params.pattern}'"
        )
        ...
        if path.is_file():
            ...
            if not params.recursive:
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Directory not empty. Use recursive=True to delete non-empty directories",
                    }
            else:
                shutil.rmtree(path)
...
def register_filesystem_tools(registry=None):
    ...
    logger.info(
        f"Filesystem tools registered: {len([t for t in Registry.tools.keys() if any(name in t for name in ['read_file', 'write_file', 'list_directory'])])} tools"
    )
    return None
